...
public class Cos implements Subroutine {
    ...
    public SExpression apply(SExpression sexp) throws LispException {
        ...
        if (theta instanceof Int) {
            result = (float) Math.cos(((Int) theta).getValue());

        } else if(theta instanceof Float) {
            result = (float) Math.cos(((Float) theta).getValue());

        } else {
            // 数値でなければエラーを投げる
            throw new LispException("手続きcosの引数は数値である必要があります");
        }
